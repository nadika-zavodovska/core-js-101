...
function isCreditCardNumber(ccn) {
  let n = ccn.toString().split('').map(Number).reverse();
  let sum = 0;

  for (let i = 0; i < n.length; i++) {
    sum += i % 2 === 0 ? n[i] : n[i] * 2 >= 10 ? n[i] * 2 - 9 : n[i] * 2;
  }
...
function getCommonDirectoryPath(pathes) {
  if (!Array.isArray(pathes) || pathes.length === 0) {
    throw new Error('Invalid input. Pathes should be a non-empty array');
  }

  if (!pathes.every(path => typeof path === 'string')) {
    throw new Error('Invalid input. All elements in the pathes array should be strings');
  }
  ...
  for (let i = 0; i < pathParts.length; i++) {
    ...
    commonPath = [commonPath, currentPart].join('/');
  }
